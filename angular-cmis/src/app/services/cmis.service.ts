...
export class CmisService {
  ...
  getRootFolder() {
    return from(this.cmisSession.getObjectByPath('/User_Homes/ad/mi/admin@gmail.com/Reading',
  ...
  getChildren(document) {
    ...
    return from(this.cmisSession.getChildren(document.id,
      { maxItems: 30,
        skipCount: skipCount,
        includeACL: true,
        includeAllowableActions: true,
  ...
  getFolderTree(document) {
    return from(this.cmisSession.getFolderTree(document.id, null,
      { includeACL: true,
        includeAllowableActions: true,
        succinct: true
      }))
